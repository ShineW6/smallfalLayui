    ...
    MobileCalendar.prototype = {
        ...
        bindEvent: function (type) {
            ...
            function popupYY(e) {
                ...
                lcalendar_cancel.addEventListener('touchstart', closeMobileCalendar);
                var lcalendar_finish = _self.gearDate.querySelector(".lcalendar_finish");
                lcalendar_finish.addEventListener('touchstart', finishMobileYY);
                var date_yy = _self.gearDate.querySelector(".date_yy");
            ...
            function popupDateTime(e) {
                ...
                _self.gearDate.innerHTML = '<div class="date_ctrl slideInUp">' +
                    ...
                    '<div class="date_btn lcalendar_finish">确定</div>' +
                    '</div>' +
                    '<div class="date_roll_mask">' +//date_roll_mask
                    '<div class="datetime_roll" style="width:4.9rem">' +//datetime_roll
                    '<div>' +///
                    '<div class="gear date_yy" data-datetype="date_yy" style="margin-left:0"></div>' +
                    '<div class="date_grid">' +
                    '<div>年</div>' +
                    '</div>' +
                    '</div>' +///
                    ...
                    '</div>' +///
                    '<div>' +///
                    '<div class="gear time_mm" data-datetype="time_mm" style="display:none"></div>' +
                    '<div class="date_grid" style="display:none">' +
                    '<div style="display:none">分</div>' +
                    '</div>' +
                    '</div>' +///
                    '</div>' + //datetime_roll
                    '</div>' + //date_roll_mask
            ...
            //年确认
            function finishMobileYY(e) {
                var passY = _self.maxY - _self.minY + 1;
                var date_yy = parseInt(Math.round(_self.gearDate.querySelector(".date_yy").getAttribute("val")));


                _self.trigger.value = (date_yy % passY + _self.minY) + "年";
                closeMobileCalendar(e);
            }
            //日期时间确认
            function finishMobileDateTime(e) {
                ...
                closeMobileCalendar(e);
                var datetime = (date_yy % passY + _self.minY) + "年" + Number(date_mm) + "月" + Number(date_dd) + "日" + Number(time_hh) + "点";
                if (typeof confirmDateTime === "function") {
                    confirmDateTime(datetime, _self);
                }
            }
            //时间确认
